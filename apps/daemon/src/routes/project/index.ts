...
// RACCOONUI-PATCH: mirror project rows to .raccoonui-project.json so
// coworkers cloning a project via git can reconstruct their local DB
// row — 2026-05-04 (moved from server.ts on 2026-05-13 after upstream
// extracted project routes into this module; path is ../../ since the
// module now lives under routes/project/ — 2026-06-25).
...
export function registerProjectRoutes(app: Express, ctx: RegisterProjectRoutesDeps) {
  ...
  app.post('/api/projects', async (req, res) => {
    try {
      ...
      // git syncs metadata alongside files — 2026-05-04 (re-injected after
      // upstream moved both the seed conversation and plugin-snapshot
      // resolution inside the insertProject transaction; those now live
      // there and reach the response via `pluginResolutionState`).
      writeProjectMetadata(PROJECTS_DIR, project).catch((err: any) => {
