...
// (writeProjectMetadata moved to routes/project/index.ts when upstream
// extracted project routes; server.ts keeps the import-side ops only.)
import {
...
export async function startServer({
  ...
  // ── RACCOONUI-PATCH BLOCK (2026-05-04…2026-05-13) ─────────────────────
  // All endpoints under the /api/raccoonui/* namespace live inline here
  // rather than in a register*Routes module — they are fork-only and not
  // intended for upstream contribution. Keeping them inline avoids
  // round-tripping every upstream route refactor through a parallel
  // raccoonui-routes module.
  ...
  app.post('/api/raccoonui/projects/:id/files/*splat/retag-anchors', async (req, res) => {
    try {
      const projectId = req.params.id;
      // RACCOONUI-PATCH: express 5 / path-to-regexp v8 dropped the bare `*`
      // wildcard; use the named `*splat` form (req.params.splat is an array
      // of path segments) matching upstream's file-route convention. — 2026-05-30
