  ...
  useEffect(() => {
    ...
    function onMessage(ev: MessageEvent) {
      ...
      if (data.type === 'od:comment-targets' && Array.isArray(data.targets)) {
        ...
        // RACCOONUI-PATCH: gate the untagged banner on a confirmed targets
        // report (distinguishes still-loading iframe from genuinely empty
        // tagging) — 2026-05-08
      ...
      <div className="viewer-body" ref={previewBodyRef}>
        {source === null && !sourceEverLoadedRef.current ? (
          ...
          >
            ...
            {/* RACCOONUI-PATCH: warn the user when comment selection is on
                but the doc has zero [data-od-id] / [data-screen-label]
                anchors — upstream's contract (since 38eb78a3) is silent
                failure on untagged HTML, which feels like a broken Picker.
                The retag button mirrors id="X" → data-od-id="X" via daemon
                endpoint so existing structural elements opt into the
                contract without round-tripping through the LLM.
                NOTE 2026-05-30: upstream #3081 (Studio mark tool) merged
                Draw+Screenshot and dropped drawClickSelectionMode; element
                selection now flows through boardMode only, so the trigger
                reverts to boardMode (freehand draw needs no anchors). — 2026-05-08 */}
