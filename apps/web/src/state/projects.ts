...
export async function createProject(
  ...
  try {
    ...
    // RACCOONUI-PATCH: honor an explicit slug before falling back to uuid — 2026-05-04
    // Upstream now also accepts `input.id`, but only rejects null/undefined; the
    // fork's slug arrives from a text field, so a blank string must still mint a uuid.
    ...
    for (let attempt = 0; ; attempt += 1) {
      const resp = await fetch('/api/projects', {
        ...
        body: JSON.stringify({ ...omitWorkspaceContext(input), id }),
      });
      if (resp.ok) {
