...
- GitHub `<your-account>/marketing-q3` (private) — repo 名 = project slug
...
gh repo edit zoe-account/design-pitch-jan --add-collaborator rindy-account
```

之後在 Slack 把 repo URL 傳給 Rindy:
```
git@github.com:zoe-account/design-pitch-jan.git
```

### Rindy 端 — clone + import

```bash
./scripts/raccoonui/clone-project.sh git@github.com:zoe-account/design-pitch-jan.git
```
...
# 然後砍
gh repo delete <your-account>/<slug> --yes
```
...
gh repo delete <你的 account>/<舊 slug> --yes
...
slug 是 user **本地 SQLite primary key**, 在不同 user 各自的 SQLite 不會撞。但 GitHub repo 名 `<slug>` 在同一個 GitHub account 下會撞 — 同帳號重用 slug 名要先砍舊 repo 或挑新 slug。如果 Zoe 跟 Rindy 各自 GitHub 都用同 slug — 沒事,因為 repo path 含 owner(`zoe/...` vs `rindy/...`)。
