...
async function discoverBrandSources(): Promise<BrandSources[]> {
  ...
  for (const entry of entries) {
    ...
    // RACCOONUI-PATCH: skip brands that don't ship tokens.css + components.html.
    // The fork-only `raccoonai` brand is DESIGN.md-driven and intentionally
    // opts out of upstream's structured-token fixture pipeline — matches the
    // existing convention in check-tokens-fixture-sync.ts. — 2026-05-19
    const [tokensRead, fixtureRead] = await Promise.all([
      readFile(tokensPath, 'utf8').catch(() => null),
      readFile(fixturePath, 'utf8').catch(() => null),
    ]);
    if (tokensRead == null || fixtureRead == null) continue;
    sources.push({
      id: entry.name,
      fixturePath,
      tokensCss: tokensRead,
      fixtureHtml: fixtureRead,
    });
  }
