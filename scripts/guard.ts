...
async function checkToolsLayout(): Promise<boolean> {
  ...
  for (const entry of entries) {
    ...
    if (expected == null) {
      violations.push(`${repositoryPath} -> tools/ top-level entries are allowlisted; expected only AGENTS.md, dev/, pack/, raccoonui/, release/, and serve/`);
      continue;
    }
